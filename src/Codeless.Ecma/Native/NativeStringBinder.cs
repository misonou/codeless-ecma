...
namespace Codeless.Ecma.Native {
  internal class NativeStringBinder : PrimitiveBinder<string> {
    public static readonly NativeStringBinder Default = new NativeStringBinder();

    protected NativeStringBinder() { }
    ...
    public override int ToInt32(string value) {
      ...
      return NativeDoubleBinder.Default.ToInt32(ToDouble(value));
    }
    ...
    public override long ToInt64(string value) {
      ...
      return NativeDoubleBinder.Default.ToInt64(ToDouble(value));
    }
    ...
    public override bool TryGet(string target, EcmaPropertyKey name, out EcmaValue value) {
      if (name.IsCanonicalNumericIndex) {
        value = name.CanonicalNumericIndex < target.Length ? new String(target[(int)name.CanonicalNumericIndex], 1) : EcmaValue.Undefined;
        return true;
      }
      if (name.Name == "length") {
        value = target.Length;
